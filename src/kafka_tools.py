...
async def get_kafka_payload(
    ...
    """
    Returns payload for specified Apache Kafka topic. Plaintext connection only (no SASL/SSL authentication)
    
    Returns payload of the first available message.
    """
    # Generate group_id if not provided
    if group_id is None:
        group_id = f'de-chat-{str(uuid4())}'
        ...
        async with asyncio.timeout(timeout_seconds):
            ...
            if msg:
                ...
                for tp, messages in msg.items():
                    if messages:
                        first_msg = messages[0]
                        return (f"Sample message offset: {first_msg.offset}\n"
                                f"Mesasge payload:\n```\n{first_msg.value}\n```\n")
