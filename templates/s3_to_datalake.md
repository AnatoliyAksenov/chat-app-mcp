...
```py
'''
DAG: s3_messages_to_datalake_daily
Description: Daily only new ETL of messages data from PostgreSQL to data lake
Schedule: 0 6 * * * (Midnight UTC daily)
Owner: team01@lct.dev.com
Tags: ['etl', 's3', 'datalake', 'daily', 'new-load']
'''
...
DAG_ID = generate_dag_name('s3_messages_to_datalake_daily', __file__)
...
connection = BaseHook.get_connection('s3_messages')
...
sql_cmd = generate_spark_sql(DAG_ID, sql_file=f'{dag_path}/sql/messages.sql', spark_configs=conf, gitlab_token=gitlab_token)
...
cmd = generate_spark_submit_command(DAG_ID, script_path=f"{dag_path}/tasks/spark_messages_etl.py", spark_configs=conf, gitlab_token=gitlab_token)
...
dq_cmd = generate_spark_submit_command(DAG_ID, script_path=f"{dag_path}/tasks/dq_messages.py", spark_configs=conf, gitlab_token=gitlab_token)
